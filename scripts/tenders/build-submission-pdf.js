...
function normalizeTypography(s) {
  // Some portals/PDF viewers choke on certain Unicode typography glyphs.
  // Keep this conservative: fix the usual culprits without touching currency symbols like £.
  return String(s ?? '')
    .replace(/\u00A0/g, ' ') // NBSP
    .replace(/\u2018|\u2019/g, "'") // ‘ ’
    .replace(/\u201C|\u201D/g, '"') // “ ”
...
function mdToHtml(md) {
  ...
  // - fenced code blocks
  // - tables (pipe tables)
  const lines = normalizeTypography(String(md || '')).replace(/\r\n/g, '\n').split('\n');
