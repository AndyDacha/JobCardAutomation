...
function buildComplianceMatrixUosVms(info, evidence) {
  ...
  const rows = [
    ...
    },
    {
      ...
      resp: 'Capabilities addressed at solution level; detailed requirement-by-requirement response is completed in File 6 (with supporting notes/screenshots).',
      ev: 'tender-response-pack.md §4; functional-nonfunctional-response-notes.md',
      status: '⚠️ Requires clarification'
    },
    {
      clause: 'File 5 §8.1(c)',
...
function main() {
  ...
  writeText(
    ...
    mk(`Tender Submission Response Pack — ${info.ref}`, [
      ...
      '- VMS platform deployed on University-provided server infrastructure (iSolutions).',
      '- RAID NVR infrastructure (24‑bay chassis) configured for resilience and retention objectives.',
      '- Video wall decoders configured for the existing 55” screen wall arrangement and operator workflows.',
      '',
      '### 4.2 Security & auditability (high-level)',
      '- Role-based access control and multi-user / multi-group permissions.',
      '- Audit trail of user activity (user ID, timestamp, action) aligned to evidential handling requirements.',
      '- Secure export of footage with integrity controls and export logging.',
      '- Hardening approach and encrypted communications aligned to University cyber expectations.',
      '',
      '### 4.3 Active Directory (MSAD) integration',
      '- User and group management integrated with MSAD for operator accounts and role-based access.',
      '',
      '### 4.4 Operator performance (joystick/PTZ)',
      '- Joystick/keypad workflows validated during demonstration including fast-moving target tracking and camera switching latency.',
      '',
  ...
  writeText(path.join(outDir, 'demonstration-plan.md'), mk('Demonstration Plan (per ITT File 2 §3.2.4.1)', [
    ...
    '|---|---|---|',
    '| 1. In-system management/config/diagnostics | Admin tools, device health, alerting, logs | Screenshare walkthrough + screenshots (optional) |',
    '| 2. Compatibility with operator equipment | CCR workflow with typical workstation spec | Operator workflow notes |',
    '| 3. Video wall configuration/operation | Layouts, salvos, tours, wall control | Live config changes + operator control |',
    '| 4. Archive/retrieve/export | Search, bookmarks, export, integrity controls | Export pack + audit log extract |',
    '| 5. MSAD user/group management | Group mapping, RBAC, least privilege | AD mapping table |',
    '| 6. Security/hardening/encrypted comms | Secure settings baseline, encryption options, logging | Hardening checklist |',
    '| 7. Existing infrastructure utilisation | Server separation, reuse of screens/peripherals where possible | Architecture diagram (logical) |',
    '| 8–9. Joystick/PTZ multi-camera tracking | Three PTZ tracking + switching latency | Operator notes + performance observations |',
    '',
